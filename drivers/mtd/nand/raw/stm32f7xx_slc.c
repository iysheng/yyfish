				  ...
				  unsigned int ctrl)
{
	...
	tmp = readb(SLC_CFG(host->io_base));
	if (ctrl & NAND_NCE)
		tmp |= SLCCFG_CE_LOW;
	else
		tmp &= ~SLCCFG_CE_LOW;
	writeb(tmp, SLC_CFG(host->io_base));
	#endif
...
static int lpc32xx_nand_device_ready(struct nand_chip *chip)
{
	...
	if ((readb(SLC_STAT(host->io_base)) & SLCSTAT_NAND_READY) != 0)
		rdy = 1;
			...
			int read)
{
	...
	if (read) {
		writeb(readb(SLC_CFG(host->io_base)) |
	...
	/* Clear initial ECC */
	writeb(SLCCTRL_ECC_CLEAR, host->ncfg->iobase_cmd);

	/* Transfer size is data area only */
	writeb(mtd->writesize, SLC_TC(host->io_base));

	/* Start transfer in the NAND controller */
	writeb(readb(host->ncfg->iobase_cmd) | SLCCTRL_DMA_START,
	       host->ncfg->iobase_cmd);
	 ...
	 */
	if (readb(SLC_STAT(host->io_base)) & SLCSTAT_DMA_FIFO) {
		dev_warn(mtd->dev.parent, "FIFO not empty!\n");
		timeout = jiffies + msecs_to_jiffies(LPC32XX_DMA_TIMEOUT);
		while ((readb(SLC_STAT(host->io_base)) & SLCSTAT_DMA_FIFO) &&
	...
	host->ecc_buf[chip->ecc.steps - 1] =
		readb(SLC_ECC(host->io_base));

	/* Flush DMA */
	dmaengine_terminate_all(host->dma_chan);

	if (readb(SLC_STAT(host->io_base)) & SLCSTAT_DMA_FIFO ||
	    readb(SLC_TC(host->io_base))) {
...
static int lpc32xx_nand_remove(struct platform_device *pdev)
{
	...
	/* Force CE high */
	tmp = readb(host->ncfg->iobase_cmd);
	tmp &= ~SLCCFG_CE_LOW;
	writeb(tmp, host->ncfg->iobase_cmd);
